...
def check_price_is_live_and_is_float(app: IBapi, stock_request_id: int) -> None:
    ...
    if not isinstance(app.current_asset_price_dict[stock_request_id].price, list):
        ...
        if np.sum(np.array(app.current_asset_price_dict[stock_request_id].price) < 0) != 0:  # type: ignore
