...
def check_price_is_live_and_is_float(app: IBapi, stock_request_id: None | int) -> None:
    ...
    if not app.stock_current_price_dict[stock_request_id].market_is_live:
        raise PriceNotLiveError("Terminating process due to price not being live.")

    if not isinstance(app.stock_current_price_dict[stock_request_id].price, list):
        raise PriceNotFloatError(f"Terminating process due to price not being of float type: "
                                 f"got type {type(app.stock_current_price_dict[stock_request_id].price)}")

    else:
        if np.sum(np.array(app.stock_current_price_dict[stock_request_id].price) < 0) != 0:  # type: ignore
            raise PriceNotFloatError(f"Terminating process due to price being negative: "
                                     f"got price {app.stock_current_price_dict[stock_request_id].price}")
